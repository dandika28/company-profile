...
public class PasswordGenerateUtil {
	...
	public static HashSalt createNewHashWithSalt(String password)
	...
	public static String hashPassword(PasswordSalt passwordSalt)
			throws NoSuchAlgorithmException, InvalidKeySpecException {

		KeySpec spec = new PBEKeySpec(passwordSalt.getPassword().toCharArray(),
				DatatypeConverter.parseHexBinary(passwordSalt.getSalt()),
				ITERATION_COUNT, KEY_LENGTH);
		SecretKeyFactory f = SecretKeyFactory.getInstance(ALGORITHM);
		byte[] hash = f.generateSecret(spec).getEncoded();
